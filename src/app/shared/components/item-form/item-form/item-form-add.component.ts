...
import { Item, ItemPayload, Qualities, Types } from 'src/app/core/interfaces/build';
...
export class ItemFormAddComponent  implements OnInit {
  ...
  @Input() set item(_item: Item | null) {
    if (_item) {
      ...
      this.form.controls['selectedQualities'].setValue(_item.attributes.quality_id.data.id);
      this.form.controls['selectedTypes'].setValue(_item.attributes.type_id.data.id);
  ...
  updateItem() {
    if (this.form && this.form.valid && this.itemId !== null) {
      const itemData: ItemPayload = {
        item_name: this.form.get('itemname')?.value,
        quality_id: this.form.get('selectedQuality')?.value,
        type_id: this.form.get('selectedType')?.value,
      };
